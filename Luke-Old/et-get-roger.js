...
exports.aggressivedto = window.aggressivedto = aggressivedto = function aggressivedto(widInput, preamble, level, callback) { // returns a made up dto base on maximum number of relationships, etc
        //Debug='true';
		...
		executeobject["executethis"]=getwid;
		execute(executeobject, function (parameterObject){
			executegetwidcallback(parameterObject);
		});
		
		function executegetwidcallback(parameterObject){
			// ** 11-1
			...
			if ((parameterObject!==undefined) && (parameterObject['metadata.method'] != "") && (parameterObject['metadata.method'] != targetwid))  {
				...
				executeobject["executethis"]=getwid;
				execute(executeobject, function (moreDTOParameters){
					...
					executegetwidcallback2();
				});
			}else{
				executegetwidcallback2();
			}

			function executegetwidcallback2(){
				...
				executeobject["executethis"] = querywid;
				execute(executeobject, function (moreDTOParameters){
					executequerywidcallback(moreDTOParameters);
				});
				
				function executequerywidcallback(moreDTOParameters){
					...
					/*for (eachresult in moreDTOParameters) {
						moreDTOParametersLoop(eachresult);
					}*/
					
					var moreDTOParametersLength=Object.keys(moreDTOParameters).length;
					if (moreDTOParametersLength !== 0) {         
						(function recurse (cnt) {
							var eachresult=moreDTOParameters[cnt];
							moreDTOParametersLoop(eachresult);
							cnt++;
							if (cnt<moreDTOParametersLength) {recurse(cnt)};
						})( 0 ); // recurse end
					} // length	end
					
					function moreDTOParametersLoop(eachresult){
						rightparameters={};
						...
						if (level > 0) {
							parameterObject = jsonConcat(parameterObject, aggressivedto(key, key, level, function(){}))
						};
					}
...
*/
    ...
    function getcleanparameters(resultObj, dtotype, accesstoken, cleanmethod, convertmethod, callback) {
            ...
            )
        {
			...
			aggressivedto(resultObj['wid'],"",10, function (dtoobject){
				aggressivedtocallback(dtoobject);
			});
			
			function aggressivedtocallback(dtoobject){
				...
				var dtoobjectLength=Object.keys(dtoobject).length;
				if (dtoobjectLength !== 0) {         
					(function recurse (cnt) {
						var item=dtoobject[cnt];
						dtoobjectLoop(item);
						cnt++;
						if (cnt<dtoobjectLength) {recurse(cnt)};
					})( 0 ); // recurse end
				} // length	end
				
				function dtoobjectLoop(item){
					preamble="";
					proposedLeft=item;
					...
					if (proposedRight == 'inherit') {
						...
						executeobject["executethis"]=getwidmaster;
						execute(executeobject, function (moreParameters){
							...
							executegetwidmastercallback();
						});
					}else{
						executegetwidmastercallback();
					}
					
					function executegetwidmastercallback(){
...
//  var resultObj = {};
		...
		getWidMongo(wid, convertMethod, accesstoken, dtotype, function (resultObj){
			getWidMongoCallback(resultObj);
		});

		function getWidMongoCallback(resultObj){
		...
		function executegetwidcallback(){
			...
			if ((Object.keys(resultObj).length !== 0) && (resultObj['wid']!=resultObj['metadata.method'])) {
				...
				getcleanparameters(resultObj, dtotype, accesstoken, "remove", convertMethod, function (resultObj){
					resultObj=resultObj.parms;
					getcleanparameterscallback();
				});
			}else{
				getcleanparameterscallback();
			}
			
			function getcleanparameterscallback(){
				//resultObj=resultObj.parms || {};   // ************
...
//Debug=olddebug;
		...
		executeobject["executethis"]=getwid;
		execute(executeobject, function (currentLevelObject){
			executegetwidcallback(currentLevelObject);
		});

        function executegetwidcallback(currentLevelObject){
			...
			if ((currentLevelObject["metadata.method"]!==undefined) &&
				...
				executeobject["executethis"]=getwid;
				execute(executeobject, function (dtoGlobalParameters){
			...
			function executegetwidcallback(){
				...
				aggressivedto(widInput,"",1, function (moreParameters){
					aggressivedtocallback(moreParameters);
				});
				
				function aggressivedtocallback(moreParameters){
					...
					/*for (var item in dtoGlobalParameters) {
						dtoGlobalParametersLoop(item);
					} // for*/
					
					var dtoGlobalParametersLength=Object.keys(dtoGlobalParameters).length;
					if (dtoGlobalParametersLength !== 0) {         
						(function recurse (cnt) {
							var item=dtoGlobalParameters[cnt];
							dtoGlobalParametersLoop(item);							
							cnt++;
							if (cnt<dtoGlobalParametersLength) {recurse(cnt)};
						})( 0 ); // recurse end
					} // length end
					
					function dtoGlobalParametersLoop(item){
						...
						if ((attr == "onetoone")  || (attr == "onetomany")) { // 10-24 || (attr == "inherit"))  {
							...
							//nextLevelParameters = getAndFormatNextLevel(widInput, "attributes", mongorelationshipmethod, "forward", item, convertMethod, accessToken, dtoin);						// 11-9 readded inherit from cleanparms here:
							getAndFormatNextLevel(widInput, "attributes", mongorelationshipmethod, "forward", item, convertMethod, accessToken, dtoin, function (nextLevelParameters){
...
// Starting of getAndFormatNextLevel function
    function getAndFormatNextLevel(widInput, mongorelationshiptype, mongorelationshipmethod, mongorelationshipdirection, mongowidmethod, convertmethod, accesstoken, dtoin, callback) {
		...
		executeobject["executethis"]=querywid;
		execute(executeobject, function (executequerywidcallback){
			executequerywidcallback(relatedParameters);
		});
		
		function executequerywidcallback(relatedParameters){
			// ***** 10-31
			...
			/*for(iteration = 0 ; (iteration< countKeys(relatedParameters)) ; iteration++ ) {
				relatedParametersLoop(iteration);
			}*/
			
			(function recurse(iteration) {
				relatedParametersLoop(iteration);
				iteration++;
				if(iteration < countKeys(relatedParameters)) recurse(iteration);
			})(0); //recurse end
			
			function relatedParametersLoop(iteration){
				...
				if ((convertmethod == "") || (convertmethod == "dto") || (convertmethod == "toobject") ||
					...
					getWidMongo(proposedLeft, convertmethod, accesstoken, dtoin, function (drillDownParameters){
						getWidMongoCallback(drillDownParameters);
					});
					
					function getWidMongoCallback(drillDownParameters){
		...
		//                  if ((convertmethod == "dto") && (relatedParameters.length == 1)) { 
			...
			callback(nextLevelParametersObject);
		}	
    }
/*
