...
// Prepares an object to be recorded in local storage and puts it there
    function MongoAddEditPrepare(Indto, InList, widid, widdto, callback) {
        ...
        function executecallback () {
            ...
            var indtolength=Object.keys(Indto).length;
            if (indtolength !== 0) {         
                (function recurse1 (cnt) {
                    var item=Indto[cnt];
                    cnt++;
                    var executeobject={};
                        executeobject["executethis"]="getwid"; 
	...
	function AddMongoRelationship(ParentWid,ChildWid,attr,callback){
		...
		execute(executeobject, function (widset){
			...
			MongoAddEditPrepare([], widset, "", attr, function (AddedObject){
				callback();
			});
		});
    } 
...
*/
    ...
    function addcleanparameters(resultObj, dtotype, accesstoken, cleanmethod, convertmethod, callback) {
        ...
        if ((inputParametersObject['metadata.method'])) { // && (dtotype=="")) {
			...
			executeobject["executethis"]=getwidmaster;
			execute(executeobject, function (dtoobject){
...
// process for DOT notation.
    exports.addwidmaster = addwidmaster = function addwidmaster (inputObject, callback) {
		...
		AddWidParameters(OutParameters, function (Wid){
...
//		Debug=olddebug;
		...
		addcleanparameters(inputParametersObject, dtotype, accesstoken, "add", convertmethod, addcleanparameterscallback);
	
		function addcleanparameterscallback(parameterObject){
		...
		function executenext(){
			...
			AddMaster(dtoList, inputList, inputParametersObject["wid"], metadata, function(Wid){					
...
*/
    ...
    function AddMaster(dtoList, parameterList, widName, dtotype, callback) {
        ...
        //ParentObject = 
        MongoAddEditPrepare(ParentdtoList, ParentList, widName, dtotype, MongoAddEditPrepareCallback);
		...
		function MongoAddEditPrepareCallback(ParentObject){
			...
			function ChildrenListobjLoop(childrentype){
				...
				if (editflag == 'true') {				// edit means read wids before write -- to get wid names
					...
					execute(executeobject, function (widlist){
						// **** 10-31
			...
			if (Object.keys(parameterindexobj).length !== 0) {
				...
				function ParameterindexobjLoop(currentchild){
					...
					if (countKeys(ParametersToAdd)!==0) {
						...
						AddMaster(ChildrendtoList, ParametersToAdd, widtoadd, childrentype, function (ChildWid){
							...
							AddMongoRelationship(ParentWid, ChildWid, "attributes", function (){
